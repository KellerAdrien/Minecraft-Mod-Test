...
public class NaviEntity extends PathAwareEntity implements InventoryOwner, Vibrations {
    ...
    private static final TrackedData<Boolean> CAN_DUPLICATE = DataTracker.registerData(NaviEntity.class,
            TrackedDataHandlerRegistry.BOOLEAN);
    protected static final ImmutableList<SensorType<? extends Sensor<? super NaviEntity>>> SENSORS = ImmutableList.of(
    ...
    public static final ImmutableList<Float> THROW_SOUND_PITCHES = ImmutableList.of(
            0.5625F, 0.625F, 0.75F, 0.9375F, 1.0F, 1.0F, 1.125F, 1.25F, 1.5F, 1.875F, 2.0F, 2.25F, 2.5F, 3.0F, 3.75F,
            4.0F);
    ...
    public NaviEntity(EntityType<? extends NaviEntity> entityType, World world) {
    ...
    @Override
    protected Brain.Profile<NaviEntity> createBrainProfile() {
        return Brain.createProfile(MEMORY_MODULES, SENSORS);
    }

    @Override
    protected Brain<?> deserializeBrain(Dynamic<?> dynamic) {
        return NaviBrain.create(this.createBrainProfile().deserialize(dynamic));
    }

    @Override
    public Brain<NaviEntity> getBrain() {
        return (Brain<NaviEntity>) super.getBrain();
    }
    ...
    protected void mobTick() {
        ...
        NaviBrain.updateActivities(this);
        this.getWorld().getProfiler().pop();
        super.mobTick();
    }
    ...
    private boolean isItemPickupCoolingDown() {
        return this.getBrain().isMemoryInState(MemoryModuleType.ITEM_PICKUP_COOLDOWN_TICKS,
    ...
    protected ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (this.isDancing() && this.matchesDuplicationIngredient(itemStack) && this.canDuplicate()) {
            ...
            this.getWorld().playSoundFromEntity(player, this, SoundEvents.BLOCK_AMETHYST_BLOCK_CHIME,
            ...
            this.decrementStackUnlessInCreative(player, itemStack);
            this.getWorld().playSoundFromEntity(player, this, SoundEvents.ENTITY_ALLAY_ITEM_GIVEN,
                    SoundCategory.NEUTRAL, 2.0F, 1.0F);
            ...
            this.getWorld().playSoundFromEntity(player, this, SoundEvents.ENTITY_ALLAY_ITEM_TAKEN,
                    SoundCategory.NEUTRAL, 2.0F, 1.0F);
    ...
    private boolean shouldStopDancing() {
        return this.jukeboxPos == null
                || !this.jukeboxPos.isWithinDistance(this.getPos(),
                        (double) GameEvent.JUKEBOX_PLAY.value().notificationRadius())
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("listener", NbtElement.COMPOUND_TYPE)) {
            Vibrations.ListenerData.CODEC
                    ...
                    .resultOrPartial(
                            string -> LOGGER.error("Failed to parse vibration listener for Allay: '{}'", string))
    ...
    private void duplicate() {
        NaviEntity NaviEntity = ModEntities.NAVI.create(this.getWorld());
        if (NaviEntity != null) {
            NaviEntity.refreshPositionAfterTeleport(this.getPos());
            NaviEntity.setPersistent();
            NaviEntity.startDuplicationCooldown();
            this.startDuplicationCooldown();
            this.getWorld().spawnEntity(NaviEntity);
        }
    }
    ...
    private void addHeartParticle() {
        ...
        this.getWorld().addParticle(ParticleTypes.HEART, this.getParticleX(1.0), this.getRandomBodyY() + 0.5,
    ...
    class VibrationCallback implements Vibrations.Callback {
        ...
        public boolean accepts(ServerWorld world, BlockPos pos, RegistryEntry<GameEvent> event,
            ...
            if (NaviEntity.this.isAiDisabled()) {
                return false;
            } else {
                Optional<GlobalPos> optional = NaviEntity.this.getBrain()
                        .getOptionalRegisteredMemory(MemoryModuleType.LIKED_NOTEBLOCK);
                if (optional.isEmpty()) {
                    return true;
                } else {
        ...
        public void accept(ServerWorld world, BlockPos pos, RegistryEntry<GameEvent> event,
            ...
            if (event.matches(GameEvent.NOTE_BLOCK_PLAY)) {
                AllayBrain.rememberNoteBlock(NaviEntity.this, new BlockPos(pos));
            }
        }
